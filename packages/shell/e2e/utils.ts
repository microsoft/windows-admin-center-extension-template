...
export class Utils {
    ...
    public static async findElementsAsync(
        ...
        Utils.log('Find elements. Selector: ' + cssSelector + ' | text: ' + text + ' | customSelectorHandler: ' + (customSelectorHandler ? 'Yes' : 'No'));
    ...
    public static async waitForNotVisibleAsync(webElement: WebElement): Promise<void> {
        await Utils.waitAsync(
            ...
            'waitForNotVisibleAsync', false, true);
    }
    ...
    public static log(content: string): void {
        ...
        if (!Settings.logDepth || Utils.logIndent <= parseInt(Settings.logDepth, 0)) {
            console.log(content);
        }
