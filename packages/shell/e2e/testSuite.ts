...
export class TestSuite {
    ...
    public describe(description: string, specDefinition: () => void, connectionTypes: string[] = ['servers']): void {
        if (Settings.onlyList) {
            console.log(description);
            specDefinition();
        } else {
            if (Settings.skipSpecs) {
                let skipSpecs = Settings.skipSpecs.split(';');
                let shouldRun = true;
                for (let i = 0; i < skipSpecs.length; i++) {
                    if (description.toLowerCase().indexOf(skipSpecs[i].toLowerCase()) !== -1) {
                        shouldRun = false;
                        break;
                    }
                }
            ...
            if (Settings.onlySpecs) {
                let onlySpecs = Settings.onlySpecs.split(';');
                let shouldRun = false;
                for (let i = 0; i < onlySpecs.length; i++) {
                    if (description.toLowerCase().indexOf(onlySpecs[i].toLowerCase()) !== -1) {
                        shouldRun = true;
                        break;
                    }
                }
            ...
            // Sets the Jasmine test case timeout to one hour to reduce the timeout control of Jasmine.
            // Basically we don't want Jasmine timeout to impact the test case since once the timeout happens,
            // it messes up with the promises and messes up with our test case life cycle which is heavily depending on
            // those promise based async logics.
    ...
    public it(expectation: string, assertion: () => Promise<void>, retryCount = 2): void {
        if (Settings.onlyList) {
            ...
            if (Settings.onlyRun && expectation.toLowerCase().indexOf(Settings.onlyRun.toLowerCase()) === -1) {
                return;
            }
...
jasmineSpec.prototype.addExpectationResult = function (passed, data, isError) {
    if (!passed) {
        if (data.message) {
            ...
            Utils.log('Test Failure caused by implementation: ' + data.error.message);
            Utils.log('Stack: ' + data.error.stack);
        }
