    ...
    await Utils.waitAsync(
      async (): Promise<boolean> => {
        ...
        for (let i = 0; i < alerts.length; i++) {
          ...
          let screenReaderElement = await Utils.findElementAsync('.sme-screen-reader', null, alerts[i]);
          let screenReaderText = await Utils.getTextAsync(screenReaderElement);
          currentText = currentText.replace(screenReaderText + '\n', '');
      ...
      'waitForAlertByText|' + description, false, true);
