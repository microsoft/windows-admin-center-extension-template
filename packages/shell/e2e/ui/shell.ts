...
export class Shell extends BaseUI {
  private isGatewayElevated = false;
  ...
  public async goToConnectionAsync(connectionName: string, connectionType = ConnectionType.Server,
    ...
    let needToElevateGatewayProcess = false;
    await Utils.sleepAsync(2000);
    try {
      needToElevateGatewayProcess = (await this.confirmationDialog.header.getTextAsync()) === 'Elevate gateway process';
    } catch (e) {
      Utils.log('Error in elevate gateway process dialog detecting: ' + e);
    }
    if (needToElevateGatewayProcess) {
      await this.confirmationDialog.yesButton.clickAsync();

      if (!this.isGatewayElevated) {
        this.isGatewayElevated = true;
        await this.goToHomeScreenAsync();
        await this.waitForReadyAsync();
        await this.goToConnectionAsync(connectionName, connectionType, userName, password);
        return;
      }
    }
    ...
    await Utils.waitAsync(async () => {
      if (!userName && !password) {
        ...
        if (await useAnotherAccountRadioButton.isVisibleAsync()) {
          ...
          let applyToAll = this.form.getChildUIObject({ name: 'Apply To All checkbox', selector: 'sme-form-field[name="applyToAll"] input' });
      ...
      try {
        ...
        let headerText = await this.messageDialog.header.getTextAsync();
        if (headerText === 'Connection Error' || headerText === 'Couldn\'t connect') {
  ...
  public async goToToolAsync(name: string): Promise<void> {
    ...
    await Utils.waitAsync(async () => {
      ...
      return false;
    }, 'Wait for the tool item to be clicked.', false, true);

    await Utils.waitAsync(async () => {
      toolItemWebElement = await this.getToolMenuItemWebElementByName(name);
      let isActive = (await Utils.getAttributeAsync(toolItemWebElement, 'className')).indexOf('active') !== -1;
      return isActive;
    }, 'Wait for the tool menu item is actually clicked.', false, true);
  ...
  private async getToolMenuItemWebElementByName(name: string): Promise<WebElement> {
    let toolItemWebElement = await this.toolList.findElementAsync('a', null, async webElement => {
      ...
      Utils.log('Find tool text: ' + text);

      let screenReaderElement = await Utils.findElementAsync('.sme-screen-reader', '', webElement);
      let screenReaderText = '';
      if (screenReaderElement) {
        screenReaderText = await Utils.getTextAsync(screenReaderElement);
      }

      text = text.replace(' (side loaded)', '').replace(screenReaderText + '\n', '');
      let found = name === text;
      if (found) {
        Utils.log('Found the tool.');
      }
      return found;
    });
