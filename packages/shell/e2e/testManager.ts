...
export class TestManager<T extends BaseUI> {
    ...
    public async startTestCaseAsync(): Promise<void> {
        ...
        if (Settings.testResourceConfig) {
            ...
            await Utils.retryAsync(
                async () => {
                    ...
                    let testURL = this.getTestURL();
                    Utils.log('Test URL: ' + testURL);
        ...
        if (Settings.browser === 'chrome' && (Settings.collectConsoleLogs || Settings.collectNetworkTraffic)) {
    ...
    public async goToConnectionAndToolAsync(connectionName: string, toolName: string, toolFrameSrcKeyword?: string,
        ...
        userName = userName || Settings.testUsername;
        password = password || Settings.testPassword;
        ...
        await Utils.sleepAsync(2000) // Just cannot immediately switch to tool frame since at that moment, the active iframe may not be switched to the target tool yet.
